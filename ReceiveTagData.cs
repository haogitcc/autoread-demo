...
namespace ReceiveAutonomousReading
{
    class ReceiveTagData
    {
        ...
        {
            Console.WriteLine("Trying to connect...");
            Console.WriteLine(string.Format("connecting to {0}", port));
            baseTime = DateTime.MinValue;
            _serialPort = new SerialPort();
            _serialPort.PortName = port;
            _serialPort.WriteTimeout = 1000;
            ...
            foreach (int baudRate in bps)
            {
                try
                {
                    ...
                    byte[] response = new byte[256];
                    response = receiveMessage(0x22, 1000);
                    Console.WriteLine("Connection to {0},{1} is successful", port, baudRate);
                    return true;
                }
                catch (Exception)
                {
                    _serialPort.Close();
                    Console.WriteLine("Connection to {0} is failed!", baudRate);
                    continue;
                }
            }
            Console.WriteLine(string.Format("connect to {0} is failed", port));
            return false;
        }
        ...
        public void Disconnect()
        {
            ...
            {
                _serialPort.Close();
                Console.WriteLine("disconnect {0},{1}", _serialPort.PortName, _serialPort.BaudRate);
            }
            ...
            try
            {
                ...
                do
                {
                    ...
                    for (sohIndex = 0; sohIndex < (messageLength - 2); sohIndex++)
                    {
                        ...
                        {
                            if (inputBuffer[sohIndex + 2] == 0x22)
                            {
            ...
            while (keepReceiving)
            {
                ...
                catch (Exception ex)
                {
                    if (ex is TimeoutException || ex is IOException || ex.Message.Equals("The port is closed."))
                    {
                        ...
                        {
                            _serialPort.Close();
                            Console.WriteLine("_serialPort close cause Exception!");
                        }
                        catch (Exception) { };
                    }
        ...
        {
            //Console.WriteLine("ProcessStreamingResponse {0}", ByteFormat.ToHex(response, "", " "));
            try
            {
                if (null != response)
                {
                    ...
                    {
                        byte responseType = response[((response[5] & (byte)0x10) == 0x10) ? 11 : 9];
                        //Console.WriteLine("responseType : {0}", responseType);
                        ...
                        if (br.parseResponse)
                        {
                            ...
                            else
                            {
                                ...
                                if (((response[5] & 0x88) == (byte)0x88) && (response[3] == 0x04) && (response[4] == 0x00))
                                {
                                    Double elapsedTime = Convert.ToDouble(ByteConv.ToU32(response, 12));
                                    baseTime = baseTime.AddMilliseconds(elapsedTime);
                                    return;
                                }
                                TagReadData t = new TagReadData();
                                t._baseTime = baseTime;
                                int readOffset = 5 + 7; //start of metadata response[11]
            ...
            {
                Console.WriteLine("ProcessStreamingResponse Exception: {0}", ex.Message);
        ...
        {
            //Console.WriteLine("ParseTagData {0}", ByteFormat.ToHex(response, "", " "));
        ...
        private class ResponseCallback
        {
            ...
            public ResponseCallback(byte[] response)
            {
                _response = response;
            }
